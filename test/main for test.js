...
class discountFix extends Discount {
    ...
    discountApply(order, currentTotal) {
        const orderPrice = getTotal(order);
        if (orderPrice >= 5000) {
            return this.value
        }
        else {
...
class promocodeDiscount extends Discount {
    ...
    discountApply(order, currentTotal) {
        const orderPrice = currentTotal;
        if (
            actualFixPromocode.includes(order.promoCode)
            && getTotal(order) >= 3000
        ) {
            return this.value
        }
        else if (
            actualProcentPromocode.includes(order.promoCode)
            && getTotal(order) >= 3000
        ) {
...
function discountEngine(order) {
    ...
    for (const discount of discounts) {
        const discountValue = bankersRound(
            discount.discountApply(order, finalTotal)
        );
        if (discountValue > 0) {
